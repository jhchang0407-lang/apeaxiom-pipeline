"""Consumer Staples-specific KPIs from SEC XBRL data.

Covers: packaged food/beverage, household/personal products, tobacco.
Key metrics: gross margin (brand power), SGA leverage, organic growth proxy,
inventory turnover, cash conversion, dividend growth, ROIC, goodwill intensity.
"""
...
def compute_consumer_staples_kpis(gaap: dict, years: int = 5) -> dict:
    # ── Revenue ─────────────────────────────────────────────────────────
    ...
    for i, date in enumerate(rev_dates[:years]):
        ...
        # ── SGA Leverage (marketing/promo spend proxy) ──────────────────
        ...
        # Inventory turnover
        inv_turnover = safe_div(cg or cogs_by_date.get(date), inv)
        ...
        # ── Goodwill Intensity (M&A footprint) ──────────────────────────
        ...
        computed.append({
            "date": date,
            # Margins (brand power indicators)
